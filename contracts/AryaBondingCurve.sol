...
/**
 * @title AryaBondingCurve
 * @dev Linear bonding curve for ARYA token on Base
 * 
 * Formula: price = a * supply + b
 * 
 * Buy: User sends ETH → receives ARYA
 * Sell: User sends ARYA → receives ETH
 * 
 * At threshold (e.g., 75K ETH market cap), can migrate to Uniswap V4 pool
 */
contract AryaBondingCurve is ReentrancyGuard, Ownable {
    
    IERC20 public aryaToken;
    
    // Curve parameters
    uint256 public constant A = 0.00001 ether;  // Slope
    ...
    uint256 public migrationThreshold = 75 ether; // 75 ETH threshold
    ...
    constructor(address _aryaToken) {
        aryaToken = IERC20(_aryaToken);
        totalSupply = 1000000 * 10**18; // Initial supply 1M
    }
    ...
    function buy(uint256 minTokens) external payable nonReentrant returns (uint256) {
        ...
        require(aryaToken.transfer(msg.sender, tokensToBuy), "Transfer failed");
    ...
    function sell(uint256 tokenAmount, uint256 minEth) external nonReentrant returns (uint256) {
        ...
        require(aryaToken.transferFrom(msg.sender, address(this), tokenAmount), "Transfer failed");
    ...
    function migrate() external onlyOwner {
        ...
        // Transfer remaining ARYA to pool
        uint256 aryaBalance = aryaToken.balanceOf(address(this));
        if (aryaBalance > 0) {
            aryaToken.transfer(uniswapPool, aryaBalance);
        }
