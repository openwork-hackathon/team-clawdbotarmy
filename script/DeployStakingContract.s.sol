...
contract DeployStakingContract is Script {
    ...
    function run() external {
        ...
        address openworkToken = vm.envAddress("OPENWORK_TOKEN", OPENWORK_TOKEN);
        address rewardToken = vm.envAddress("REWARD_TOKEN", ARYA_TOKEN);
        ...
        if (balance < 0.01 ether) {
            console2.log("⚠️  Warning: Low balance! Need at least 0.01 ETH for deployment");
        }
        ...
        console2.log("📄 Contract:", address(staking));
        console2.log("");
        console2.log("Next steps:");
        console2.log("1. Verify on Basescan:");
        console2.log("   npx hardhat verify --network base", address(staking), ARYA_TOKEN, openworkToken, rewardToken);
